...
import os
from Vulnerability_detection_tool.DataPreprocessing import DefactcodeDataset,count_max_seq_len
from DetectMModel.loacal_tokennize.LocalTokenizer import loadtokenizer, word2index
print(torch.cuda.is_available())

# C:\Users\xincun\Desktop\graduation_project_v4\DetectMModel\TransformerModel\tokenize_dict.txt
tokenizer_dict = loadtokenizer(r"DetectMModel\TransformerModel\tokenize_dict.txt")
max_seq_length = count_max_seq_len(r"DetectMModel\TransformerModel\trainsformer_datast.txt",tokenizer_dict)
src_len =tgt_len=max_seq_length #从构造的数据集中确认序列的最大长度
tgt_vocab_size=src_vocab_size = len(tokenizer_dict.keys())
print("huqinsong",max_seq_length,src_vocab_size)

class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=700): #max_len可以定位预测句子的最大长度
...
def train():
    model.train()
    tokenizerdict_path = r"DetectMModel\TransformerModel\tokenize_dict.txt"
    loader = Data.DataLoader(DefactcodeDataset(tokenizerdict_path,\
                r"DetectMModel\TransformerModel\data\train.txt", max_seq_length), 15, True)
    accuracy_list=[]
    loss_list=[]
    batch_index_list=[]
    ...
    torch.save(model.state_dict(), r"DetectMModel\TransformerModel\local_model\transformer.pth")
...
def defactcode_detect(model_path,defactcode_setction):
    ...
    model.eval()
    # print("预测模型加载完毕")
    defactcodeindex_section=torch.LongTensor(word2index(defactcode_setction,tokenizer_dict,max_seq_length)).to("cuda")
    # print("输入的缺陷代码序列化的结果为：",defactcodeindex_section)
    ...
    # print("生成贪婪词序为：",greedy_dec_input)
    predict, _, _, _=model(defactcodeindex_section.view(1,-1), greedy_dec_input)
    # print("预测的结果为：",predict)
    # print("预测结果的形状为：",predict.shape)
    predict = predict.data.max(1,keepdim=True)[1]
    # print("huqinsong",predict)
...
if __name__ == '__main__':
    ...
    # test =(r"mov eax, 1;test eax, eax;je 0x401941;mov dword ptr [ebp - 4], 0x423088 ")
    ...
    defactcode_detect(r"DetectMModel\TransformerModel\local_model\transformer.pth",defactcode_setction)
